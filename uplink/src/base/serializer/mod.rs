...
impl StorageHandler {
    ...
    fn select(&mut self, stream: Arc<StreamConfig>) -> &mut Storage {
        self.map.entry(stream).or_insert_with(|| Storage::new(default_file_size()))
    }
    ...
    fn next(
        ...
        for (stream, storage) in storages {
            match storage.reload_on_eof() {
                ...
                Ok(true) => continue,
                Ok(false) => return Some((stream, storage)),
...
impl<C: MqttClient> Serializer<C> {
    ...
    async fn crash(
        ...
        let storage = self.storage_handler.select(stream.clone());
...
fn construct_publish(data: Box<dyn Package>) -> Result<Publish, Error> {
    ...
    let topic = data.config().topic.clone();
...
mod test {
    ...
    fn disk_to_crash() {
        ...
        match tokio::runtime::Runtime::new()
            .unwrap()
            .block_on(serializer.slow(publish, stream))
            .unwrap()
        {
    ...
    fn catchup_to_crash_with_persistence() {
        ...
        let mut storage =
            serializer.storage_handler.map.entry(stream).or_insert(Storage::new(1024));
