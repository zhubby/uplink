...
impl From<(Arc<StreamConfig>, ClientError)> for MqttError {
    fn from((stream_config, e): (Arc<StreamConfig>, ClientError)) -> Self {
        match e {
            ClientError::Request(r) => MqttError::Send(stream_config, r),
...
impl MqttClient for AsyncClient {
    ...
    where
        V: Into<Vec<u8>> + Send,
    {
        self.publish(&stream_config.topic, qos, retain, payload)
            .await
            .map_err(|e| (stream_config, e))?;
        Ok(())
    }
...
impl<C: MqttClient> Serializer<C> {
    ...
    async fn crash(
        ...
        let storage = self.storage_handler.select(stream_config.clone());
    ...
    async fn slow(
        ...
        let publish = send_publish(self.client.clone(), publish.topic, payload, stream_config);
        tokio::pin!(publish);
        ...
        let v: Result<Status, Error> = loop {
            select! {
                ...
                o = &mut publish => match o {
                    ...
                    Err(MqttError::Send(stream_config, Request::Publish(publish))) => {
                        break Ok(Status::EventLoopCrash(publish, stream_config));
                    },
                    Err(e) => {
    ...
    async fn catchup(&mut self) -> Result<Status, Error> {
        ...
        let (stream_config, storage) = match self.storage_handler.next(&mut self.metrics) {
            Some(s) => s,
        ...
        let send = send_publish(client, publish.topic, publish.payload, stream_config.clone());
        tokio::pin!(send);
        ...
        let v: Result<Status, Error> = loop {
            select! {
                ...
                o = &mut send => {
                    ...
                    let client = match o {
                        Ok(c) => c,
                        Err(MqttError::Send(stream_config, Request::Publish(publish))) => break Ok(Status::EventLoopCrash(publish, stream_config)),
                    ...
                    send.set(send_publish(client, publish.topic, payload, stream_config.clone()));
                }
        ...
        loop {
            select! {
                data = self.collector_rx.recv_async() => {
                    ...
                    match self.client.try_publish(&stream_config.topic, QoS::AtLeastOnce, false, publish.payload) {
                        Ok(_) => {
                            self.metrics.add_batch();
                            self.metrics.add_sent_size(payload_size);
                            continue;
                        }
                        Err(MqttError::TrySend(Request::Publish(publish))) => return Ok(Status::SlowEventloop(publish, stream_config)),
                        Err(e) => unreachable!("Unexpected error: {}", e),
                    }
    ...
    pub async fn start(mut self) -> Result<(), Error> {
        ...
        loop {
            let next_status = match status {
                ...
                Status::SlowEventloop(publish, stream_config) => {
                    self.slow(publish, stream_config).await?
                }
                Status::EventLoopReady => self.catchup().await?,
                Status::EventLoopCrash(publish, stream_config) => {
                    self.crash(publish, stream_config).await?
                }
            };
...
async fn send_publish<C: MqttClient>(
    ...
    client.publish(stream_config, QoS::AtLeastOnce, false, payload).await?;
    Ok(client)
}
...
mod test {
    ...
    impl MqttClient for MockClient {
        ...
        where
            V: Into<Vec<u8>> + Send,
        {
            let mut publish = Publish::new(&stream_config.topic, qos, payload);
            publish.retain = retain;
            let publish = Request::Publish(publish);
            self.net_tx
                .send_async(publish)
                .await
                .map_err(|e| MqttError::Send(stream_config, e.into_inner()))?;
            Ok(())
        }
    ...
    impl MockCollector {
        fn new(data_tx: flume::Sender<Box<dyn Package>>) -> MockCollector {
            let config = StreamConfig { topic: "hello/world".to_owned(), ..Default::default() };
            MockCollector {
                stream: Stream::new("hello", config, 1, data_tx, Compression::Disabled),
            }
        }
    ...
    fn disk_to_crash() {
        ...
        match tokio::runtime::Runtime::new()
            .unwrap()
            .block_on(serializer.slow(publish, stream_config))
            .unwrap()
        {
